...
class Dehaze(nn.Module):
    def __init__(self):
        ...
        res2net101.load_state_dict(torch.load('./imagenet_model/res2net101_v1b_26w_4s-0812c246.pth'))
        ...
        self.tail1 = nn.Sequential(nn.ReflectionPad2d(3), nn.Conv2d(28, 3, kernel_size=7, padding=0), nn.Tanh())
    def forward(self, input):
        x, x_layer1, x_layer2 = self.encoder(input)

        x_mid = self.mid_conv(x)
        z = torch.flatten(self.avgpool(x_mid), 1)
...
class fusion_refine(nn.Module):
    ...
    def forward(self, input):
        ...
        feature,z = self.feature_extract(input)
